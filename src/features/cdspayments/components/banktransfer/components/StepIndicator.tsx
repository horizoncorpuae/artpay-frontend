...
const StepIndicator = ({ currentStep, stepConfig, className = "" }: StepIndicatorProps) => {
  const getStepStatus = (step: PaymentFlowStep) => {
    ...
    if (status === 'completed') {
      return (
        <svg
          ...
          className="size-4">
          <path
            d="M8 16C12.4183 16 16 12.4183 16 8C16 3.58172 12.4183 0 8 0C3.58172 0 0 3.58172 0 8C0 12.4183 3.58172 16 8 16ZM7.2 11.6L12 6.8L10.8 5.6L7.2 9.2L5.2 7.2L4 8.4L7.2 11.6Z"
            fill="#42B396"
          />
        </svg>
      );
    ...
    return (
      <span
        className={`block rounded-full size-3 ${
          status === 'active' ? 'bg-primary' : 'bg-[#CDCFD3]'
        }`}
      />
    );
  };
  ...
  return (
    <ul className={`ps-1.5 ${className}`}>
      {stepConfig.map(({ step, title, description }, index) => {
        ...
        return (
          <li
            key={step}
            className={`leading-5 ${
              !isLastStep ? 'border-l border-[#CDCFD3] pb-8' : ''
            } before:absolute before:content-[""] before:-left-4 before:translate-x-1/2 relative ps-4 flex items-start gap-2`}>
            ...
            <div className="flex-1">
              ...
              {status === 'completed' && step === PaymentFlowStep.CONFIRMATION && (
